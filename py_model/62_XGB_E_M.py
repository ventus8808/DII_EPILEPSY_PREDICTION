...
from scipy.stats import chi2  # Add this line
...
def main():
    ...
    metrics_dir = '/Users/maguoli/Documents/Development/Predictive/Model metrics'
    os.makedirs(metrics_dir, exist_ok=True)
    
    # Load model
    with open('/Users/maguoli/Documents/Development/Predictive/Models/62_XGB_model.pkl', 'rb') as f:
        model = pickle.load(f)
    ...
    ece, mce, bin_metrics = calculate_calibration_metrics(y_test, y_prob, weights_test)
    brier = np.average((y_prob - y_test) ** 2, weights=weights_test)
    
    # Calculate Hosmer-Lemeshow test
    observed = np.array([m['bin_total'] * m['actual_prob'] for m in bin_metrics])
    expected = np.array([m['bin_total'] * m['predicted_prob'] for m in bin_metrics])
    predicted_probs = np.array([m['predicted_prob'] for m in bin_metrics])
    
    # Calculate H-L statistic
    hl_stat = np.sum(((observed - expected) ** 2) / (expected * (1 - predicted_probs + 1e-10)))
    df = len(bin_metrics) - 2  # degrees of freedom = number of groups - 2
    p_value = 1 - stats.chi2.cdf(hl_stat, df)
    ...
    metrics_file_path = os.path.join(metrics_dir, f'{model_name}_metrics.json')
