...
def main():
    ...
    # 检查是否有已存在的最佳参数
    best_params_file = model_dir / 'LightGBM_best_params.json'
    if best_params_file.exists():
        ...
        try:
            ...
            logger.info(f"Initial best score from LightGBM_best_params.json: {best_score:.3f}")
        ...
        for trial in tqdm(range(n_trials), desc='Optuna Trials'):
            ...
            try:
                ...
                if score > best_score:
                    ...
                    param_path = model_dir / 'LightGBM_best_params.json'
                    ...
                    # 保存最佳指标
                    metrics_path = model_dir / 'LightGBM_best_metrics.json'
