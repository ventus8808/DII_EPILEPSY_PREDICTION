...
def main():
    ...
    parser = argparse.ArgumentParser(description="随机森林模型评估与可视化")
    ...
    # RF可能没有专门的特征信息文件，我们尝试从模型中获取或使用所有特征
    try:
        ...
        # 尝试从其他模型的特征信息中获取
        try:
            for model_prefix in ['CatBoost', 'XGBoost', 'LightGBM', 'SVM']:
                try:
            ...
            # 如果还是找不到，使用除目标和权重外的所有数值列
            numeric_cols = df.select_dtypes(include=np.number).columns.tolist()
            features = [col for col in numeric_cols if col not in ['Epilepsy', 'WTDRD1']]
            print("未找到特征信息文件，使用所有数值特征")
    ...
    # 预测
    print(f"成功加载模型，使用特征数：{len(features)}")
