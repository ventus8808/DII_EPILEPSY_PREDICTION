...
def main():
    ...
    ensemble_dir = model_dir  # 不再使用单独目录，直接使用model_dir
    ...
    # 加载模型
    model_path = model_dir / 'Voting_model.pkl'
    ...
    # 模型内部名称与文件名保持一致，图表显示名称更为简洁
    model_name = "Voting"
    display_name = "Voting"  # 用于图表显示的名称
    ...
    if draw_roc:
        start_time = time.time()
        print("绘制ROC曲线...")
        plot_roc_curve(y_test, y_prob, weights_test, display_name, plot_dir, plot_data_dir)
        ...
        # 构建比较字典
        y_probs_dict = {
            f"{display_name}(all feature)": y_prob_with_dii,
            f"{display_name}(without DII)": y_prob_no_dii
        }
        
        # 调用比较函数，在测试集上进行比较，不使用SMOTE过采样
        plot_roc_curve_comparison(y_test, y_probs_dict, weights_test, display_name, plot_dir, plot_data_dir, use_smote=False)
        print(f"ROC比较曲线绘制完成 (耗时 {time.time() - start_time:.2f}秒)")
    
    if draw_pr:
        start_time = time.time()
        print("绘制PR曲线...")
        plot_pr_curve(y_test, y_prob, weights_test, display_name, plot_dir, plot_data_dir)
    ...
    if draw_calibration:
        ...
        try:
            ...
            plot_calibration_all_data(y, y_prob_all, weights, display_name, plot_dir, plot_data_dir)
            # 检查文件是否正确保存，确保大小写一致
    ...
    if draw_confusion:
        ...
        plot_confusion_matrix(y_test, y_pred, display_name, plot_dir, plot_data_dir, normalize=False)
    ...
    if draw_threshold:
        ...
        print("绘制阈值曲线...")
        # 确保使用一致的大小写形式
        plot_threshold_curve(y_test, y_prob, display_name, plot_dir, plot_data_dir)
        # 如果保存的文件使用的是小写文件名，手动复制一份为大写文件名
    ...
    if draw_dca:
        ...
        # 构建DII对比字典
        y_probs_dict = {
            f"{display_name}(all feature)": y_prob_ensemble,
            f"{display_name}(without DII)": y_prob_no_dii
        }
        
        # 绘制DII贡献对比DCA曲线 - 现在直接返回数据而不是路径
        comparison_data = plot_dca_curve_comparison(y_full, y_probs_dict, weights_full, display_name, plot_dir, plot_data_dir, use_smote=True)
        
        # 创建单模型格式数据
        single_model_data = {
            "thresholds": comparison_data["thresholds"],
            "net_benefits_model": comparison_data["models"][f"{display_name}(all feature)"],
        ...
        plot_dca_curve(y_full, y_prob_ensemble, weights_full, display_name, plot_dir, plot_data_dir, use_smote=True)
