...
def plot_calibration_all_data(y_true, y_prob, weights, model_name, plot_dir, plot_data_dir, n_bins=30, use_smote=True):
    ...
    if use_transform:
        if transform_type == 'loglog':
            ...
            # 绘制直线
            ax.plot(x_ref, y_ref, '--', color='gray', alpha=0.8, linewidth=1.2, label='Perfectly calibrated', dashes=(5, 3))
        ...
        # 不使用变换，直接绘制普通直线
        ax.plot([0, 1], [0, 1], '--', color='gray', alpha=0.8, linewidth=1.2, label='Perfectly calibrated', dashes=(5, 3))
    ...
    # 设置简约风格的图例到图外右侧，并减小图例大小
    # 使用更直接的方式指定Monaco字体
    font_props = font_manager.FontProperties(family='Monaco', size=8)
