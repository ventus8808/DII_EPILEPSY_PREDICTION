...
# 加载模型、标准化器和编码器
# 首先尝试加载temp目录中的66_SVM模型
temp_dir = Path('temp')
model_path = temp_dir / '66_SVM_model.pkl'
results_path = temp_dir / '66_SVM_results.json'

# 备用路径
backup_model_path = model_dir / 'SVM_best_model.pkl'
...
try:
    ...
    try:
        ...
        print(f"temp目录中模型文件不存在: {str(e)}")
        print(f"尝试从 {backup_model_path} 加载模型...")
        with open(backup_model_path, 'rb') as f:
            model = pickle.load(f)
        print(f"成功从备用路径加载SVM模型")
